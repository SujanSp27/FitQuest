...
const SocialIcon = ({ href, label, path }) => (
  <IconButton
    ...
    target="_blank"
    rel="noreferrer"
    sx={{ color: 'text.secondary', '&:hover': { color: 'primary.main', transform: 'translateY(-2px)' }, transition: 'all .2s ease' }}
    aria-label={label}
  >
...
const Footer = () => (
  <Box mt="80px" sx={{ background: 'linear-gradient(180deg, #ffffff 0%, #f2fbff 100%)' }}>
    <Stack gap="16px" sx={{ alignItems: 'center' }} flexWrap="wrap" px="40px" pt="32px">
      <img src={Logo} alt="logo" style={{ width: '60px', height: '48px' }} />
      <Typography variant="subtitle1" sx={{ color: 'text.secondary' }}>Train Smart. Live Strong.</Typography>
      <Stack direction="row" gap={1}>
        ...
        <SocialIcon label="LinkedIn" href="https://linkedin.com/" path="M4.98 3.5C4.98 4.88 3.86 6 2.5 6S0 4.88 0 3.5 1.12 1 2.5 1s2.48 1.12 2.48 2.5zM.5 8h4V24h-4zM8.5 8h3.8v2.2h.06c.53-1 1.84-2.2 3.78-2.2 4.04 0 4.78 2.66 4.78 6.1V24h-4v-7.1c0-1.7-.03-3.88-2.37-3.88-2.38 0-2.75 1.86-2.75 3.77V24h-4z" />
      </Stack>
    </Stack>
    <Typography variant="body2" sx={{ color: 'text.secondary' }} mt="16px" textAlign="center" pb="24px">
